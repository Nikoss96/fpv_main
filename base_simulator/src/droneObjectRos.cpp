...
void DroneObjectROS::initROSVars(ros::NodeHandle &node) {
  ...
  isPosctrl = false;
  isVelMode = false;
  pubTakeOff = node.advertise<std_msgs::Empty>("/p2176/quadcopter/takeoff", 1024);
  pubLand = node.advertise<std_msgs::Empty>("/p2176/quadcopter/land", 1024);
  pubReset = node.advertise<std_msgs::Empty>("/p2176/quadcopter/reset", 1024);
  pubPosCtrl = node.advertise<std_msgs::Bool>("/p2176/quadcopter/posctrl", 1024);
  pubCmd = node.advertise<geometry_msgs::TwistStamped>("/p2176/quadcopter/cmd_vel", 1024);
  pubVelMode = node.advertise<std_msgs::Bool>("/p2176/quadcopter/vel_mode", 1024);
}
